<template>
    <div v-if="isVisible" class="flex justify-center items-center fixed inset-0 z-50">
        <div
            class="banner flex items-center transform transition-all duration-500 ease-out scale-90 opacity-0 w-4/5 max-w-xl bg-gradient-to-r
from-[#d8485b56] to-[#e7df1242] bg-opacity-40 backdrop-blur-xl p-4 border border-white border-opacity-10 rounded-lg shadow-lg">
            <div class="flex-grow text-3xl font-semibold !text-gray">
                <slot>
                    <p class="m-4">{{ message }}</p>
                </slot>
            </div>
            <button @click="dismissBanner" class="absolute top-2 right-2 bg-transparent text-[#e7df1242] rounded cursor-pointer p-0 border-none
                hover:text-[#a97c1bb9] animate-pulse">
                <svg width="24px" height="24px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"
                    class="text-current" stroke-width="1.5">
